...
const ProductTile = (props) => {
    ...
    const [fetchProductDetails, setFetchProductDetails] = useState(false)
    const [selectedProductId, setSelectedProductId] = useState(null) 
    const [productDetails, setProductDetails] = useState(null)

    useEffect(() => {
        
        if (fetchProductDetails && selectedProductId) {
            console.log("product detailsss :",fetchProductDetails , selectedProductId)
            // Fetch product data when quick view is triggered
            const fetchData = async () => {
                const { data } = await useProduct(
                    {
                        parameters: {
                            id: selectedProductId, // Use selectedProductId
                            allImages: true
                        }
                    },
                    {
                ...
                setProductDetails(data) // Store the fetched product data
                onOpen() // Open QuickView once the data is fetched
            }
            fetchData()
        }
    ...
    const handleQuickView = (productId) => {
        console.log("producttt :",productId)
        setSelectedProductId(productId?.representedProduct?.id) // Set the selected productId
        setFetchProductDetails(true)   // Trigger fetching of product details
        onOpen()
    }
